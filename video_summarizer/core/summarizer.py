...
def summarize_text(text, length_level=3, include_chapters=True):
    ...
    if os.getenv('OPENAI_API_KEY'):
        try:
            print("Attempting to summarize with OpenAI...")
            return summarize_with_openai(text, length_level, include_chapters)
        except Exception as e:
            print(f"Warning: OpenAI summarization failed: {str(e)}")
            print("Falling back to local summarization...")
    else:
        print("OpenAI API key not found, using local summarization...")
    
    # Fall back to transformers (note: the local method doesn't support chapter summaries)
    try:
        if include_chapters:
            print("Note: Chapter-based summarization is only available with OpenAI. Providing standard summary instead.")
        return summarize_with_transformers(text, length_level)
    except Exception as e:
        print(f"Warning: Local summarization failed: {str(e)}")
